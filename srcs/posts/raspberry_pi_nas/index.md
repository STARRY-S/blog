...
一定要使用功率足够大的电源适配器为树莓派供电，如果需要为树莓派连接多块移动硬盘，请一定要使用带供电的usb连接线，否则会出现很严重的问题：<br>
（供电不足导致的数据传输中断，严重的会导致硬盘受损）
</div>
...
在分区时除了`boot`和`root`之外，我额外分了1G的`swap`分区，树莓派1G内存有些小不过只是搭个人用的NAS的话实际上是不影响使用的。（~~这话咋读着这么别扭呢~~）

需要注意的是，内存卡的第一个分区一定要为`boot`，第二个分区为`root`，最后一个分区是`swap`，顺序不要乱了不然是无法开机的。

最后咱按照安装教程，把内存卡格式化成了酱紫：

``` text
Device     Boot    Start      End  Sectors  Size Id Type
/dev/sdc1           2048   411647   409600  200M  c W95 FAT32 (LBA)
/dev/sdc2         411648 60235775 59824128 28.5G 83 Linux
/dev/sdc3       60235776 62333951  2098176    1G 82 Linux swap / Solaris
```

> 卸载内存卡之前，记得随手敲几遍`sync`
...
可以参考北京外国语大学开源软件镜像源的[帮助页面](https://mirrors.bfsu.edu.cn/help/archlinuxarm/)。
...
# 北京交通大学
Server = https://mirror.bjtu.edu.cn/archlinuxarm/$arch/$repo
```

之后执行`pacman -Syyu`更新系统。
...
安装好`samba`安装包后，需要手动去`/etc/samba/`创建`smb.conf`配置文件，可以复制这个样例文件：[smb.conf.default](https://git.samba.org/samba.git/?p=samba.git;a=blob_plain;f=examples/smb.conf.default;hb=HEAD)，咱只需要把它粘贴再简单修改一下就好。
...
添加一个现有的Linux用户到Samba中：

```
# 如果你没有重启树莓派，记得先挂载硬盘
$ sudo mkdir /samba
$ sudo mount /dev/sdXn /samba

$ sudo mkdir -p /samba/sharefolder
$ sudo smbpasswd -a username                        # 添加用户并设置密码
$ sudo chown username:username /samba/sharefolder   # 修改文件夹的所属用户和组
```
