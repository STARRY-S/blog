...
不用树莓派的原因是，它是 ARM 架构的卡片机，Arch Linux 官方只支持 x86_64 架构的系统，Arch Linux ARM 准却来说实际上是个第三方的系统。
...
本来想把整体预算控制在 2K 以内的，但实际上光主板 + CPU 就两千多了……
...
> 装机的过程没怎么拍照，先放一两张图，等以后有机会再拍几张补回来。

{{< figure src="images/001.jpg" title="主板 + CPU" >}}

映泰的这块板子是不带无线网卡的，但送了一个 WIFI5 的无线网卡，需要手动安装上去，当时废了九牛二虎之力才接上了这两根 SMA 线……。

{{< figure src="images/002.jpg" title="无线网卡" >}}
...
在计网课程中，老师曾反复强调路由器和交换机的区别，路由器是 OSI 七层模型中的网络层的设备，而交换机是第二层的数据链路层的设备，但光是这么讲的话，死记硬背是能记住这两个设备之间的区别，但这种知识实在过于抽象，很难真正的理解。在网上搜这方面资料的时候看到有人把路由器称作“3 层交换机”，而常说的那种交换机则称为“2 层交换机”。说实话我也没彻底的搞明白数据链路层和网络层以及路由器和交换机之间的具体区别，不过往简单了说，可以把路由器看成是一个根据 IP 地址分发数据的设备，可以在不同的网段之间转发数据，而交换机是通过 MAC 地址分发数据的设备，再往详细了讲说实话我也讲不明白。

然后修改 PCIE 网卡的 4 个接口 `enp3s0` - `enp6s0` 的 MAC 地址和板载网口 `enp9s0` 的 MAC 地址一致，这样5个网口和虚拟的 `br0` 网口都使用同一个 MAC 地址，就能实现交换机的功能了，至于为啥要把 5 个网口的 MAC 地址都设置一致这个别问我，我也讲不明白，如果这里有哪些知识点有误，可以评论告诉我。
...
然后安装 `samba`，在 `/etc/samba/` 目录下创建 `smb.conf`，具体的过程请参照 [Wiki](https://wiki.archlinux.org/title/Samba)，在此不过多赘述。


## 其他

目前组装的 NAS 还只是个半成品，才刚开始 24 小时不断电运行，因为没插硬盘所以晚上睡觉时只能听到非常细微的风扇转动的声音（移动硬盘设置了没有工作的情况会自动停转所以没声音），后续如果有什么更新我再写到这里，时间有限先写这么多。
